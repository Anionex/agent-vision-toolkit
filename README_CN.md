...
本仓库做的不是把图片交给多模态模型拿一段通用描述回来就完事，在描述的针对性、多图并发、跨轮缓存上都做了处理，具体见下面的亮点。
...
固定的"请描述这张图"——图转文字方案通常就发这一句——不只是丢焦点，它还会诱导模型把空白填满。同一张图、同一个视觉模型，各调一次：

图是一张图标对照表，每组左边是原图标、右边是描摹输出。若干描摹是坏的，还有一格是空的——那次描摹什么都没生成。

- **固定 prompt**：把右列称作"更大更清晰的版本"，然后给那个空格子写了一句 *"faint grey dashed lines forming the partial outline of a box"*（虚线组成的半截方框）。那里什么都没有。
- **带 focus hint**——模型上一句刚说想看看哪些图标坏了：开头就是 *"several of the traced icons are severely broken"*，每一组都标出 `Reference` / `Traced (Broken)`，并如实报告那个空格子没有对应的描摹结果。

hint 给的是任务框架，不是强调权重。被要求成对核验时，模型必须回答"另一半在哪"，空缺就会以空缺的形式暴露出来；只被要求描述时，它是在讲述画面——遇到空白就用旁边的东西补上。
