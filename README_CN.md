...
Codex -> 127.0.0.1:19100 -> 用户原有的 DeepSeek 上游
             |
             +-- 请求含图片时：
                 focus hint（图片前方最近的用户文字）
                   -> 按场景路由的视觉 prompt -> 文字描述 -> 替换图片
```

视觉 prompt 不是固定的"请描述这张图"。代理会把用户的提问作为 **focus hint** 随图一起交给视觉模型，让描述围绕用户真正关心的内容展开，并按 hint 关键词路由到场景模板：报错截图逐字转写堆栈和行号，UI 截图输出元素清单（文字/位置/颜色），图表读出数据点，其余情况详细描述并逐字转写全部可见文字。描述按（图片, prompt）缓存；由于 hint 取自不可变历史中图片**前方**的文字，同一张图只描述一次，之后每轮都命中缓存。
