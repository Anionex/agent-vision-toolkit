...
不只是"多模态模型给一段 caption"：描述会围绕这一轮真正在问的东西展开，还配了定位、盘点、测量的工具——见[亮点](#亮点)。
...
朴素的桥接是固定发一句"请描述这张图"，再把 caption 塞回文本模型。这里的目标不一样：让纯文本模型真正看清楚，能把视觉任务做完。

- **问题导向的描述，不是通用 caption**：每张图都带一条 *focus hint*——贴图用它自己那条消息的文字，`view_image` 取回的图用模型自述的看图动机。描述因此围绕这一轮要用到的细节展开，而不是在讲壁纸什么颜色。
- **视觉模型是眼睛，不是大脑**：它只转写和描述，不替你回答问题，推理留给你的编程模型——基于图本身推理，而不是基于别人的二手结论。
- **不会对没看到的图硬推**：某张图描述失败就直接报错，不会静默塞占位符，也不会把原图丢给纯文本模型。
- **两种图片结构都支持**：直接粘贴（`message.content`）和模型调用 `view_image` 的返回（`function_call_output.output`）。
- **N 张图约等于 1 张图的延迟，同一张图只描述一次**：多图并发；描述按（图片, prompt）缓存，而两种 hint 都取自不可变的对话历史，这个组合会稳定复现，所以多步任务的后续每一轮都命中缓存。

**光有描述是有损的**——文字给不出可点击的坐标和精确几何。剩下的部分交给可选的独立 CLI，任何纯文本 agent 都能用：
